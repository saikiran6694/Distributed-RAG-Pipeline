...
from query.hallucination_detector import HallucinationDetector
...
class QueryEngine:
    ...
    async def stream(self, request: QueryRequest) -> AsyncIterator[GeneratorChunk]:
        ...
        final_chunks, prompt, _ = await self._retrieve_and_build(request)

        full_text = ""
        final_chunk = None
        async for gen_chunk in self._generator.stream(prompt, request.history):
            full_text += gen_chunk.delta
            if not gen_chunk.done:
                yield gen_chunk
            else:
                final_chunk = gen_chunk

        groundedness = None
        if full_text and prompt.citations:
            citations_with_text = [
                {
                    **c,
                    "text": next(
                        (
                            ...
                            if str(ch.chunk_id) == str(c.get("chunk_id", ""))
                        ),
                        "",
                    ),
                }
            ...
            result = await self._detector.check(full_text, citations_with_text)
            groundedness = result.to_dict()

        if final_chunk:
            final_chunk.groundedness = groundedness
            yield final_chunk
    ...
    async def query(self, request: QueryRequest) -> QueryResponse:
        ...
        final_chunks, prompt, retrieval = await self._retrieve_and_build(request)
        ...
        # Run hallucination detection after generation (non-blocking on failure)
        groundedness = None
        if full_text and prompt.citations:
            citations_with_text = [
                {
            ...
            result = await self._detector.check(answer=full_text, citations=citations_with_text)
            groundedness = result.to_dict()
